...
public class CostCalculationService {
    ...
    public BigDecimal calculateCost(List<ElectricityReading> electricityReadings, PricePlan pricePlan) {
        ...
        averagedCost = averagedCost.setScale(1,BigDecimal.ROUND_HALF_UP);
        return averagedCost.multiply(pricePlan.getUnitRate()).setScale(1,RoundingMode.HALF_UP);
    }
    ...
    public List<ElectricityReading> calculateReadingsBySpecifiedTime(List<ElectricityReading> electricityReadings,Integer numOfDays) {
        return electricityReadings.stream()
                .filter(electricityReading -> electricityReading.getTime().isAfter(Instant.now().minus(numOfDays, ChronoUnit.DAYS)))
